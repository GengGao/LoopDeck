...
function parseJsonData(data: unknown): ImportResult {
  ...
  if (Array.isArray(data)) {
    ...
    for (let i = 0; i < data.length; i++) {
      try {
        const item = normalizeToReviewItem(
...
function convertOpenAIFormat(
  ...
  return {
    ...
    outputs: choices.map((choice, index) => ({
      ...
      token_usage: Number(
        (data.usage as Record<string, unknown>)?.total_tokens || 0,
      ),
      latency_ms: 0,
    })),
    human_feedback: {},
...
function convertOpenAIFineTuneFormat(
  data: Record<string, unknown>,
  now: string,
): ReviewItem {
...
function convertGenericFormat(
  ...
  if (
    ...
    if (inputObj.context_chunks && Array.isArray(inputObj.context_chunks)) {
      for (const chunk of inputObj.context_chunks as Array<
      ...
      >) {
        ...
        inputContextChunks.push({
          ...
          source: String(chunk.source || metadata?.source || "unknown"),
          score: Number(
            chunk.score || chunk.similarity || chunk.relevance_score || 0,
          ),
          metadata: metadata,
        });
      }
  ...
  const response = String(
    data.response || data.answer || data.output || data.completion || "",
  );

  // Extract additional context if available (not already extracted from input object)
  const contextChunks: ContextChunk[] =
    inputContextChunks.length > 0 ? inputContextChunks : [];
  const contextField =
    data.context || data.contexts || data.documents || data.chunks;

  if (Array.isArray(contextField)) {
    for (const chunk of contextField as Array<
    ...
    .map((item) => {
      ...
      const responseText =
        ...
        (item.human_feedback.selected_model_id
          ? item.outputs.find(
              (o) => o.model_id === item.human_feedback.selected_model_id,
            )?.text
          : item.outputs[0]?.text) ||
        "";
    ...
    .map((item) => {
      ...
      if (item.input.system_prompt) {
        exported.messages.push({
          role: "system",
          content: item.input.system_prompt,
        });
      }
